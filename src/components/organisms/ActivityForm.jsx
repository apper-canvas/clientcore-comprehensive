...
const ActivityForm = ({ activity, onSubmit, onCancel, isSubmitting = false }) => {
  ...
  useEffect(() => {
    if (activity) {
      setFormData({
contact_id_c: activity.contact_id_c?.Id?.toString() || "",
        deal_id_c: activity.deal_id_c?.Id?.toString() || "",
...
const contactDeals = deals.filter(deal => deal.contact_id_c?.Id?.toString() === formData.contact_id_c);
