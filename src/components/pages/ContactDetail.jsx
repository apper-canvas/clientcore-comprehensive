...
setContact(contactData);
      setDeals(allDeals.filter(deal => deal.contact_id_c?.Id === contactData.Id));
      setActivities(allActivities.filter(activity => activity.contact_id_c?.Id === contactData.Id));
    } catch (err) {
...
<span className="text-sm">
                    Added {format(new Date(contact.created_at_c), "MMM dd, yyyy")}
                  </span>
                </div>
              </div>
                    ...
                    <div className="space-y-4">
                      {activities.slice(0, 3).map(activity => (
                        <ActivityItem
                          ...
                          activity={activity}
                          contact={contact}
deal={activity.deal_id_c ? getDealById(activity.deal_id_c.Id) : null}
                        />
                      ))}
                      {activities.length > 3 && (
                        <Button
